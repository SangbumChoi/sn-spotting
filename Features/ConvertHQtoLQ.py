...
def convert_video(input_path, output_path):
    ...
    try:
        ...
        ff = ffmpy.FFmpeg(
            inputs={input_path: ""},
            outputs={
                output_path: "-r 25 -vf scale=-1:224 -ss "
                + str(start)
                + " -t "
                + str(stop)
            },
...
def elaborateVideo(args, half):
    ...
    if not os.path.exists(Half_crop224_MKV_file):
        ...
        if os.path.exists(Half_MKV_file):
            convert_video(Half_MKV_file, Half_crop224_MKV_file)
        elif os.path.exists(Half_MP4_file):
            convert_video(Half_MP4_file, Half_crop224_MKV_file)
        elif os.path.exists(Half_TS_file):
            convert_video(Half_TS_file, Half_crop224_MKV_file)
...
game_list = random.sample(
    game_list, len(game_list)
)  # shuffle games to run multiple instances of conversions
